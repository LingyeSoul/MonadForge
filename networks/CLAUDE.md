...
- **LoKR** (`lokr.py::LoKRModule`) — Kronecker-factor adapter. Use `lokr_full_factor=true` to keep both factors full while retaining normal `network_alpha / network_dim` scaling. The retired `network_dim=114514` sentinel is rejected for new training because it suppresses output and gradients; `lokr_allow_legacy_dim=true` exists only for old-state resume.
