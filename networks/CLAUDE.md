...
- **LoHa** (`loha.py::LoHaModule`) — thin lifecycle wrapper over `lycoris.modules.loha.LohaModule` (same 3.4.0 pin): `ΔW = (w1a@w1b) ⊙ (w2a@w2b) * (alpha/rank)`, effective rank up to r². Selected via `use_loha`; canonical `hada_*` keys, shares the LoKR train-time `split_fused_projections` path + channel-scaling exclusion. **The LoKr and LoHa 3.4.0 fixes differ** — LoHa's bypass already includes `self.scale` (don't re-add it), while `get_diff_weight` double-scales in both (both overridden); the functional loha bypass is broken as shipped (never call it). See `docs/methods/loha.md`.
