...
| 监控数据同步修复 | — | — | [必须修复] 双根因：① server.py _dir_mtime_key 的 jobs/logs 路径用 dirname+`..`×3 多跳一层（落到 model-train/），目录恒 isdir=False → key 恒空 → 索引永不刷新；修复为 dirname×3（与 discovery.MONADFORGE_ROOT 一致）+ 深入 job 目录内 progress.jsonl/stdout.log/job.json/network_train 的 mtime。② api_run/api_live 不刷新 index，且 progress.jsonl 是内容追加（目录 mtime 不变），监控 2s 轮询拿到的永远是旧 Run.jsonl 快照（实测 jsonl step 330 vs API 162）；新增 _freshen_run（按请求重解析单 run 的 jsonl+stdout，实测 3.3ms）挂到 api_run/api_live。验证：steps 560→562→564 实时增长、浏览器监控页 fill 39.39→39.67% 推进、零 JS 错误 |