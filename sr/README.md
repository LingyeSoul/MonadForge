...
The sidecar runs in the **root Anima venv** — there is no separate per-sidecar virtualenv.
`make sr-setup` selects an install profile:

- `SR_SETUP_PROFILE=auto` (default) detects the active GPU/runtime. V100 uses an
  additive install; other GPUs use the standard locked group.
- `SR_SETUP_PROFILE=v100` preserves the local `torch 2.10.0+cu129`,
  `torchvision 0.25.0+cu129`, ORT 1.26 and V100 FlashAttention stack, installing
  only missing SR packages with `uv pip install`.
- `SR_SETUP_PROFILE=standard` keeps the normal `uv sync --group sr --inexact`
  path for BF16-capable GPUs and the regular FlashAttention wheel.

The heavy metrics closure (`pyiqa` → `opencv-python-headless`, `bitsandbytes`,
`facexlib`, `datasets`, plus dev-tools as runtime deps) stays out of the default
install through the `sr` group. Use `--dry-run` with either profile to inspect the
resolver without changing the environment.

The standard profile uses the root project's Blackwell torch (`torch 2.12 + cu132`)
and regular FlashAttention wheel. The V100 profile never lets that sync replace its
validated local stack. Python 3.13 is required by both profiles.

SR/RSD GPU entry points accept `--amp_dtype auto|fp16|bf16`. `auto` resolves to
FP16 on pre-Ampere GPUs such as V100 (`sm_70`) and keeps BF16 on GPUs with native
BF16 support; pass an explicit dtype only when intentionally overriding that policy.
Training snapshots, progress logs, checkpoints, and inference summaries record the
resolved dtype.
...
```bash
make sr-setup                         # auto profile
SR_SETUP_PROFILE=v100 make sr-setup   # force V100-safe additive install
SR_SETUP_PROFILE=standard make sr-setup  # force normal BF16/FlashAttention install
make sr-prep         # build frozen synthetic-LR eval set from image_dataset/ (--n 30)
make sr-detect-text  # CTD text-region boxes for crop sampling (not OCR transcription)
...
(committed), so there's nothing to re-apply. `sr-detect-text` finds text regions only;
it does not perform OCR transcription.
