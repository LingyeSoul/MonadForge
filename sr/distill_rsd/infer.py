...
def _sample_tile(cfg, diff, vqgan, student, hr_t, align, device, amp=True,
    ...
    Heavy matmuls run under the resolved AMP dtype (FP16 on V100, BF16 on newer GPUs);
    the returned tile is cast back to fp32 for seam-averaging.

    The VAE is the VRAM peak (full pixel-res conv stack). It runs in the same native
    AMP dtype *outside* autocast, while GroupNorm still reduces in fp32 internally.
...
def student_sr(cfg, diff, vqgan, student, lq_img, device, align=256, overlap=None,
               ...
               cond_mode="latent", amp_dtype=torch.bfloat16):
...
def main():
    ...
    ap.add_argument("--no_bf16", action="store_true",
                    help="legacy alias: disable AMP and use FP32 VAE/student inference")
    ...
    amp_dtype_name = M.amp_dtype_name(amp_dtype) if not args.no_bf16 else "fp32"
    ...
    if not args.no_bf16:
        # Native resolved AMP dtype — halves the full-res GroupNorm activations that
        # autocast would otherwise keep in fp32 (the VRAM peak).
    ...
    print(f"weights: {key or 'raw'} | noise_mode: {student.noise_mode}"
          f"({student.noise_channels}ch) | cond_lq: {cond_mode} | amp_dtype: {amp_dtype_name}")
    student.eval()
    if args.compile:
