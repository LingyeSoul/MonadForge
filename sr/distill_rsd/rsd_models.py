...
def resolve_amp_dtype(requested="auto", device=DEVICE):
    """Resolve SR AMP dtype, using FP16 on GPUs without native BF16.

    Volta (including the local V100 sm_70) has no native BF16 arithmetic. PyTorch
    can emulate BF16 there, but it is slower and does not provide the intended
    tensor-core path. ``auto`` therefore follows the hardware capability while
    explicit ``fp16``/``bf16`` remains available for controlled experiments.
    """
    if requested not in {"auto", "fp16", "bf16"}:
        raise ValueError(f"amp dtype must be auto, fp16, or bf16; got {requested!r}")
    if requested == "fp16":
    ...
    try:
        ...
        # Keep the standard BF16 default when the probe is unavailable (for
        # example --help or a CPU-only static environment).
        pass
    return torch.bfloat16


def amp_dtype_name(dtype):
    """Stable metadata spelling for a resolved torch dtype."""
