...
def main():
    ...
    ap.add_argument("--amp", action=argparse.BooleanOptionalAction, default=True,
                    help="mixed-precision autocast on the nets + native AMP-dtype VQGAN "
                         "(auto selects fp16 on pre-Ampere/V100 and bf16 on newer GPUs). "
                         "--no-amp = full fp32.")
    ap.add_argument("--amp_dtype", choices=["auto", "fp16", "bf16"], default="auto",
                    help="AMP dtype: auto=fp16 on sm<8/V100, bf16 on sm>=8; explicit "
                         "fp16/bf16 overrides the hardware choice.")
    ...
    args = ap.parse_args()
    amp_dtype = M.resolve_amp_dtype(args.amp_dtype)
    amp_dtype_name = M.amp_dtype_name(amp_dtype) if args.amp else "fp32"
    ...
    if args.amp:
        # Native AMP dtype keeps the full-res VQGAN GroupNorm activations bounded.
        # On V100 this is FP16; newer BF16-capable GPUs keep the previous BF16 path.
        vqgan = vqgan.to(amp_dtype)
    ...
    ckpt_meta = {"noise_mode": student.noise_mode, "noise_channels": student.noise_channels,
                 ...
                 "amp": args.amp, "amp_dtype": amp_dtype_name}
    snapshot = {
        "method": "rsd", "version": args.version,
        "config": str(config_path), "teacher": str(teacher_ckpt),
        "cond_lq": cond_mode, "iters": args.iters, "bs": args.bs,
        "grad_accum": args.grad_accum, "amp": args.amp,
        "amp_dtype": amp_dtype_name, "compile": args.compile,
        "grad_ckpt": args.grad_ckpt,
    }
    (save_dir / "snapshot.toml").write_text(
        "\n".join(f'{key} = {json.dumps(value)}' for key, value in snapshot.items()) + "\n"
    )
    ...
    def autocast():
        return torch.autocast("cuda", dtype=amp_dtype) if args.amp else nullcontext()
    ...
    def encode(gt, lq):
        # one fused 2B encode (native AMP dtype under --amp); latents come back fp32 so all
    ...
    print(f"training: iters={args.iters} K={args.K} bs={args.bs}x{args.grad_accum} "
          f"T={T} nodes={args.nodes} amp={args.amp} amp_dtype={amp_dtype_name} "
    ...
    for step in range(args.iters):
        ...
        for _ in range(args.grad_accum):
            ...
            # decode runs native AMP dtype outside autocast (grad flows through the frozen
