...
def main():
    ...
    ap.add_argument("--amp_dtype", choices=["auto", "fp16", "bf16"], default="auto",
                    help="AMP dtype: auto=fp16 on sm<8/V100, bf16 on sm>=8")
    ...
    def sample_latents():
        ...
        with autocast():
            z0 = vqgan.encode(gt.to(vdt)) * cfg.diffusion.params.scale_factor
            z_y = vqgan.encode(lq.to(vdt)) * cfg.diffusion.params.scale_factor
    ...
    def checked_total(label, losses):
        bad = [name for name, value in losses.items() if not torch.isfinite(value).all()]
        if bad:
            raise RuntimeError(f"{label} produced non-finite losses: {bad}")
        values = " ".join(f"{name}={float(value.detach()):.6g}"
                          for name, value in losses.items())
        print(f"  [{label} losses] {values}")
    ...
    with torch.no_grad(), autocast():                              # DMD: teacher & fake no-grad
    ...
    with autocast():
        ...
        x0_img = vqgan.decode(z0_single.to(next(vqgan.parameters()).dtype),
                              force_not_quantize=True).clamp(-1, 1)
    ...
    generator_total = checked_total("generator", {
        "dmd": L_theta,
        "lpips": 2.0 * L_lpips,
        "gan": 3e-3 * L_gan_g,
    })
    ...
    print(f"\n=== RSD dry-run OK at bs={B} ({amp_dtype_name}) ===")
    report("FINAL PEAK")
