...
Runs in the root venv (modern torch, no xformers). ResShift inference is
invoked as the importable ``sr.scripts.sr_infer`` module so repository imports
such as ``library`` resolve consistently.
"""
import argparse
import json
import subprocess
...
def main():
    ...
    ap.add_argument("--amp_dtype", choices=["auto", "fp16", "bf16"], default="auto",
                    help="SR AMP dtype: auto=fp16 on sm<8/V100, bf16 on sm>=8")
    ap.add_argument("--no_amp", action="store_true", help="disable SR AMP autocast")
    ...
    if not args.skip_infer:
        cmd = [
            sys.executable, "-m", "sr.scripts.sr_infer",
