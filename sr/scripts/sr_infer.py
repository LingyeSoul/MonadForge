...
class ResShiftInfer:
    ...
    def __init__(self, version="v3", chop_size=512, chop_stride=-1, chop_bs=1,
                 use_amp=True, amp_dtype="auto", seed=12345, device=M.DEVICE,
                 ckpt_path=None, cond_lq=None):
        self.device = device
        self.use_amp = use_amp
        self.amp_dtype = M.resolve_amp_dtype(amp_dtype) if use_amp else torch.float32
        self.amp_dtype_name = M.amp_dtype_name(self.amp_dtype) if use_amp else "fp32"
    ...
    def _process(self, im_lq):
        ctx = (torch.autocast("cuda", dtype=self.amp_dtype)
    ...
    def run(self, in_path, out_path, musiq=True, sheet=True, sheet_max=32):
        ...
        summary = {"n": len(rows), "version": self.version, "ckpt": self.ckpt_name,
                   "chop": self.chop_arg, "amp": self.use_amp,
...
def main():
    ...
    ap.add_argument("--no_amp", action="store_true", help="disable AMP autocast")
    ...
    sampler = ResShiftInfer(version=args.version, chop_size=args.chop_size,
                            ...
                            ckpt_path=args.ckpt, cond_lq=args.cond_lq)
