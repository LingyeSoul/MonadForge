...
Three memory-isolated phases: encode every prompt with the text encoder
...
the VAE wants several GB to itself at 1024.
    ...
    python -m scripts.qwen21.generate \
...
from library.qwen21.requests import GenerateRequest, model_paths
...
def run_generate(req: GenerateRequest) -> Path:
    ...
    pipe = build_pipeline(paths, None, te, None, load_scheduler(paths.scheduler))
    te_attached = place(
        te,
        TEXT_ENCODER_BLOCKS,
        device,
