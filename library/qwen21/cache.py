...
never sees them: this uses phase-split loading (text
...
from library.qwen21.requests import CONFIG_DIR, CacheRequest, ModelPaths, model_paths
...
def cache_text(
    ...
    te = load_text_encoder(paths.text_encoder, torch.bfloat16)
    # Encoding does not use a diffusion schedule; do not require DiT assets.
    pipe = build_pipeline(
        paths, None, te, None, load_scheduler(CONFIG_DIR / "scheduler.json")
    )
    attached = place(
        te,
        TEXT_ENCODER_BLOCKS,
...
def cache_latents(
    ...
    pipe = build_pipeline(
        paths,
        load_vae(paths.vae, torch.bfloat16),
        None,
        None,
        load_scheduler(CONFIG_DIR / "scheduler.json"),
    )
    pipe.vae.to(device)
...
def run_cache(req: CacheRequest) -> Path:
    ...
    print(f"{len(pairs)} pairs from {src}, model {paths}", flush=True)
    ...
    if not req.skip_text:
        cache_text(pairs, out_dir, paths, device, req.te_blocks_to_swap, req.overwrite)
