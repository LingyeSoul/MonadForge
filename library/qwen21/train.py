...
``accel.compile_blocks``. ``--no-compile`` for a quick smoke.
    ...
    python -m scripts.qwen21.train --epochs 8
"""
...
from library.qwen21.requests import TrainRequest, model_paths
...
class CacheItem(TrainingBatch):
    stem: str
    size: str


def load_cache(cache_dir: Path) -> list[CacheItem]:
    ...
    for latent_path in sorted(cache_dir.glob("*.latent.safetensors")):
        ...
        if not te_path.exists():
            raise FileNotFoundError(
                f"Missing text cache for {latent_path}: {te_path}; run qwen21-cache before training"
            )
        with safe_open(latent_path, framework="pt") as handle:
            meta = handle.metadata() or {}
            latents = handle.get_tensor("latents")
        if "latent_h" not in meta or "latent_w" not in meta:
            raise SystemExit(
        ...
        if (
            ...
            raise ValueError(
                f"Invalid Qwen-Image-2.1 latent cache {latent_path}: shape={tuple(latents.shape)}, grid={latent_h}x{latent_w}; expected packed (H*W, 64) with positive even H/W"
            )
        text = load_file(te_path)
        missing = {"prompt_embeds", "prompt_embeds_mask"} - text.keys()
        if missing:
            raise ValueError(
                f"Text cache {te_path} is missing required tensors: {sorted(missing)}"
            )
        embeds, mask = text["prompt_embeds"], text["prompt_embeds_mask"]
        if (
            embeds.ndim != 2
            or embeds.shape[1] != 4096
            or embeds.shape[0] == 0
            or mask.shape != embeds.shape[:1]
        ):
            raise ValueError(
                f"Invalid Qwen3-VL cache {te_path}: embeddings={tuple(embeds.shape)}, mask={tuple(mask.shape)}; expected (L, 4096) and (L,)"
            )
        if not torch.isfinite(latents).all() or not torch.isfinite(embeds).all():
            raise ValueError(
                f"Non-finite values in cached pair {latent_path}, {te_path}; rebuild the cache"
            )
        items.append(
            {
                "stem": stem,
...
def training_step(transformer, item: TrainingBatch, sigma: torch.Tensor):
...
def report_fit(blocks, attached, batch: TrainingBatch) -> None:
...
def run_train(req: TrainRequest) -> Path:
    ...
    transformer = load_transformer(paths.dit, torch.bfloat16)
    ...
    if req.activation_reserve_gb is None:
        ...
        print(
            f"activation reserve {reserve_gb:.2f} GB (--activation_reserve_gb)",
            flush=True,
        )
