...
def _argv_has_option(argv: Optional[Sequence[str]], name: str) -> bool:
    """Return whether an option was explicitly supplied on the command line."""

    values = list(sys.argv[1:] if argv is None else argv)
    prefix = name + "="
    return any(token == name or token.startswith(prefix) for token in values)


def _mark_training_budget_provenance(
    args: argparse.Namespace,
    provenance: Optional[dict[str, str]],
    argv: Optional[Sequence[str]],
) -> argparse.Namespace:
    """Record explicit-vs-default budget intent before runtime derivation.

    Argparse always materializes ``max_train_steps=1600``.  That value must not
    make an epoch-only config look like an explicit step budget, so config
    provenance and the raw CLI are inspected before the trainer builds its
    dataloader.  The private fields are excluded from resume signatures and
    are carried into snapshots/manifests by the trainer.
    """

    source_map = provenance or {}
    steps_cli = _argv_has_option(argv, "--max_train_steps")
    epochs_cli = _argv_has_option(argv, "--max_train_epochs")
    steps_config = "max_train_steps" in source_map
    epochs_config = "max_train_epochs" in source_map
    args._max_train_steps_explicit = bool(steps_cli or steps_config)
    args._max_train_epochs_explicit = bool(epochs_cli or epochs_config)
    args._max_train_steps_source = "CLI" if steps_cli else source_map.get(
        "max_train_steps", "argparse default"
    )
    args._max_train_epochs_source = "CLI" if epochs_cli else source_map.get(
        "max_train_epochs", "unset"
    )
    args._config_provenance = dict(source_map)
    return args
...
def _resolve_preset(preset: str, configs_dir: str = "configs") -> tuple[dict, str, str]:
    ...
    for cp in custom_paths:
        ...
        if os.path.exists(cp):
            ...
            return data, cp, _display_path(cp)
    available: list[str] = []
    if os.path.exists(presets_path):
        with open(presets_path, "r", encoding="utf-8") as f:
...
def _render_merged_toml(
    ...
    budget_source = getattr(args, "training_budget_source", None)
    budget_origin = getattr(args, "training_budget_origin", None)
    effective_steps = getattr(args, "effective_max_train_steps", None)
    if budget_source:
        detail = f" source={budget_source}"
        if budget_origin:
            detail += f" origin={budget_origin}"
        if effective_steps is not None:
            detail += f" effective_max_train_steps={int(effective_steps)}"
        lines.append(f"# Training budget:{detail}")
    elif getattr(args, "_max_train_steps_explicit", None) is not None:
        lines.append(
            "# Training budget: source=pending (effective steps are finalized "
            "after the dataloader is built)"
        )
    sha = _git_sha()
    if sha:
...
def refresh_config_snapshot(args: argparse.Namespace) -> Optional[str]:
    """Rewrite the canonical snapshot after runtime-derived values are known."""

    parser = getattr(args, "_config_snapshot_parser", None)
    if parser is None:
        return None
    return _write_config_snapshot(
        args,
        parser,
        dict(getattr(args, "_config_snapshot_provenance", {}) or {}),
    )
...
def read_config_from_file(
    ...
    if method is not None and not args.config_file:
        ...
        args = _mark_training_budget_provenance(args, provenance, argv)
        args._config_snapshot_parser = parser
        args._config_snapshot_provenance = dict(provenance)
    ...
    if not args.config_file:
        args = _mark_training_budget_provenance(args, {}, argv)
        args._config_snapshot_parser = parser
        args._config_snapshot_provenance = {}
        if print_config:
            import sys as _sys
    ...
    args = _mark_training_budget_provenance(args, {k: config_path for k in merged}, argv)
    args._config_snapshot_parser = parser
    args._config_snapshot_provenance = {k: config_path for k in merged}
