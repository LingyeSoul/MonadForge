...
`networks/lora_modules/loha.py` over `lycoris-lora==4.0.0`.
...
| The LoKr/LoHa bypass-scale asymmetry | output error IF the pre-4.0 LoKr `multiplier*scale` compensator form were copied to LoHa (double-scale blowup — must stay large) |
| `get_diff_weight` double-scale bug (still present upstream in 4.0.0) | official/fixed norm ratio (equals `alpha/rank` ⇒ upstream applies scale twice; the wrapper override applies it once) |
...
Baseline run: `results/20260726-1724-baseline/` — a 3.4.0-era run (git sha
`15a49e00`, wrapper-vs-official max abs err `7.6e-06`). After the
`lycoris-lora==4.0.0` upgrade the gate re-ran green with the same invariants:
`gdw_ratio = scale = 4.0` on both probes, merge-vs-fuse err `0.0`, rank
ceiling hit 12/12 capacity rows. `results/` is gitignored — re-run per
upgrade and copy the headline numbers into the PR.
