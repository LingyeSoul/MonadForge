...
class ContinuationPrepareRequest(BaseModel):
    source_job_id: str = Field(min_length=1, max_length=128)
    budget_key: Literal["max_train_epochs", "max_train_steps"]
    target: int = Field(strict=True, gt=0)


class ContinuationSubmitRequest(BaseModel):
...
async def continuation_detail(task_id: str, lang: str = "cn"):
    from webui.services.config_service import describe_config
    try:
    ...
    for field in described["fields"]:
        field["read_only"] = field["key"] != candidate["budget_key"]
    return {"candidate": candidate, "fields": described["fields"]}


@router.post("/{task_id}/continuation/prepare")
async def prepare_continuation(task_id: str, body: ContinuationPrepareRequest):
    try:
...
async def continue_training(task_id: str, body: ContinuationSubmitRequest):
    try:
        result = await daemon_client.continue_training(task_id, body.model_dump())
