  ...
  <div class="d-flex flex-column" style="height: 100%">
    ...
    <div class="flex-grow-1 overflow-auto pa-2">
      ...
      >
        ...
        <v-card-text v-if="selectedTask === task.task_id" class="pt-0">
        ...
        <v-card-actions v-if="task.state === 'running' || task.output_lines > 0">
          <v-btn
            size="x-small"
            variant="text"
            @click="selectedTask = selectedTask === task.task_id ? '' : task.task_id"
          >
            {{ selectedTask === task.task_id ? t('taskHideLogs') : t('taskShowLogs') }}
          </v-btn>
        </v-card-actions>
      </v-card>
    </div>
